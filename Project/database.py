...
class UserSeatDatabase ():
    ...
    def create_users(self):
        ...
        """
        if len(self.make_connection("SELECT name FROM sqlite_master WHERE type='table' AND name='users'")) == 0:
    ...
    def create_seats(self):
        ...
        """
        if len(self.make_connection("SELECT name FROM sqlite_master WHERE type='table' AND name='seats'")) == 0:
	        query = "CREATE TABLE seats (seat_from_flight text, seat_number text, is_reserved blob, reserved_by integer)"
	        self.make_connection(query)
	        
    def create_flights(self):
    	"""
    	creates a table to display the flights
    	"""
    	if len(self.make_connection("SELECT name FROM sqlite_master WHERE type='table' AND name='flights'")) == 0:
	        query = "CREATE TABLE flights (flightID text, company text, from text, to text, duration integer, date text, time text, prize_per_ticket integer)"
	        self.make_connection(query)
    ...
    def insert_seat(self, seat):
       with self.connection:
           self.cursor.execute("INSERT INTO seats VALUES (:seat_from_flight, :seat_number, :is_reserved, :reserved_by)", {'seat_number': seat.seat_number,
    ...
    def login(self, user_name):
    	...
    	return login data
    	"""
    	return self.make_connection("SELECT * FROM users WHERE user_name={}".format(user_name))
